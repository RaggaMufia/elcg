...
int main()
   ...
   /* the elcg random number generator uses */
   /* the LCG algorithm emulating drand48 */
   /* plus the Bays-Durham shuffle */
   llfmt *ll;               /* elcg structure */
   ll = (llfmt *) elcginit();    /* initialize the elcg generator */
   /*******************************************/
   ...
   *p = '\0';        /* terminate the text string */
   ...
   picrowsz = 3 * ((txtlen*15) + 80 + (elcgint(ll,40)));
   piccolsz = 24 + 40 + (elcgint(ll,40));
   ...
   while (txtindx < txtlen)
	 ...
	 while (col < 12)
	    ...
	    if (*fntp == '*')
	       ...
	       *picp++ = elcgpwr(ll,6);     /* 0-63 color depth */
	       *picp++ = elcgpwr(ll,6);     /* 0-63 color depth */
	       *picp++ = elcgpwr(ll,6);     /* 0-63 color depth */
	       } /* if asterisk in font */
	    col++;
